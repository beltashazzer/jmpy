...
def varchart(x: list,
    ...
    **kwargs: other parameters to pass into the jumpy.boxplot function
    ...
    if fig:
        fig = boxplot(x=strx, y=y, data=local_data, orderby=strx, legend=strl, cumprob=cumprob, fig=fig, **kwargs)
    else:
        fig = boxplot(x=strx, y=y, data=local_data, orderby=strx, legend=strl, cumprob=cumprob, **kwargs)
    ...
    # this array holds all the multple numbers for when to draw a line.  % operator is called on each of these
    ...
    for i, combo in enumerate(sorted(set(local_data[strx])), start=1):
        ...
        for j, mod in enumerate(reversed(nummods)):
            if not i % mod:
                axm.vlines(i-.5, *yvals, color='{}'.format(colors[j+1]), alpha=.5)
